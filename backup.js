...
const AlienBackup = (() => {
  ...
  const image = value => typeof value === 'string'
    && /^(?:xfiles\/|games\/(?:matrix|bond|marvel|predator|firefly)\/)?(cards|assets)\/[\w/.-]+\.(png|jpe?g|webp)$/i.test(value) && !value.split('/').includes('..');
  ...
  function validate(backup) {
    ...
    if (backup.kind === 'room') {
      ...
      if (game?.gameId && !['alien', 'xfiles', 'matrix', 'bond', 'marvel', 'predator', 'firefly'].includes(game.gameId)) invalid();

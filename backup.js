...
const AlienBackup = (() => {
  ...
  const image = value => typeof value === 'string'
    && /^(?:xfiles\/|games\/(?:matrix|bond|marvel|marvel2|dc|predator|firefly|buffy|bigtrouble)\/)?(cards|assets)\/[\w/.-]+\.(png|jpe?g|webp)$/i.test(value) && !value.split('/').includes('..');
  ...
  function validate(backup) {
    ...
    if (backup.kind === 'room') {
      ...
      if (game?.gameId && !['alien', 'xfiles', 'matrix', 'bond', 'marvel', 'marvel2', 'dc', 'predator', 'firefly', 'buffy', 'bigtrouble'].includes(game.gameId)) invalid();
